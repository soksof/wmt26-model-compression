...
| `MODELZIP_KV_CACHE_DTYPE` | `auto` | vLLM `kv_cache_dtype`; the **FP8 package sets `fp8`** in `submission.env` (baked into the artifact, not the shell) so its reported throughput/VRAM reproduce |
