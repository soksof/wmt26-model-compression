...
Optional env knobs:
  ...
  MODELZIP_KV_CACHE_DTYPE  vLLM kv_cache_dtype (default "auto"; the FP8 package sets "fp8"
                     in its submission.env so the throughput/VRAM we report reproduce cold)
    ...
    def vllm(self):
        if self._vllm is None:
            ...
            engine = LLM(
                ...
                # KV-cache dtype must live in the ARTIFACT, not the caller's shell: the FP8
                # package's reported throughput/VRAM assume fp8 KV cache, so its submission.env
                # sets MODELZIP_KV_CACHE_DTYPE=fp8. Default "auto" == prior behaviour for the
                # INT4 packages (they don't set it), so this is a no-op for them.
                kv_cache_dtype=os.getenv("MODELZIP_KV_CACHE_DTYPE", "auto"),
